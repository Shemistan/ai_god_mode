...
BLOCKED = [
    ...
    # 3a. Прочие манипуляции с main/master
    ("git branch -m main old-main", "worktree-a"),
    ("git branch -d main", "worktree-a"),
    ("git reset --soft HEAD~1", "main"),
    ("git reset HEAD~1", "master"),
    ("git update-ref refs/heads/main abc123", "worktree-a"),
    ("git symbolic-ref HEAD refs/heads/main", "worktree-a"),
    ("git push --all origin", "worktree-a"),
    ("git push --mirror origin", "worktree-a"),
    # 3a. Смена ветки внутри составной команды не снимает защиту main
    ("git switch main && git commit -m 'feat: x'", "worktree-a"),
    ("git checkout main; git merge worktree-a", "worktree-a"),
    ("git checkout master && git reset --soft HEAD~1", "worktree-a"),
    ("git switch main && git push origin HEAD", "worktree-a"),
    # 3b. main защищён и на стороне GitHub (gh)
    ("gh pr merge 3 --squash", "worktree-a"),
    ("gh pr merge --auto --merge 3", "worktree-a"),
    ("gh repo delete Shemistan/ai_god_mode --yes", None),
    ("gh api -X PATCH repos/o/r/git/refs/heads/main -f sha=abc", "worktree-a"),
    ("gh api --method POST repos/o/r/merges -f base=main -f head=staging",
     "worktree-a"),
    ("gh api -X DELETE repos/o/r/branches/main/protection", "worktree-a"),
    # 4. Секреты
    ("cat .env", None),
...
ALLOWED = [
    ...
    ("./finish", "worktree-x"),
    # Полномочия агента: ветки, staging, push, PR
    ("git branch staging main", "worktree-x"),
    ("git switch -c worktree-x/sub staging", "worktree-x"),
    ("git checkout -b worktree-x/sub2 staging", "worktree-x"),
    ("git push origin staging", "worktree-x"),
    ("git push -u origin worktree-x/sub", "worktree-x/sub"),
    ("git switch -c feat staging && git commit -m 'feat: x'", "main"),
    ("git checkout main && git log --oneline -5", "worktree-x"),
    ("git checkout main -- README.md", "worktree-x"),
    ("git branch --contains main", "worktree-x"),
    ("gh pr create --base main --head staging --title 'x' --body 'y'",
     "worktree-x"),
    ("gh pr list --base main --head staging --state open --json number",
     "worktree-x"),
    ("gh pr view 3 --json url -q .url", "worktree-x"),
    ("gh pr edit 3 --body 'обновлено'", "worktree-x"),
    ("gh pr status", "worktree-x"),
    ("gh auth status", "worktree-x"),
    ("gh api repos/o/r/pulls", "worktree-x"),
    ("gh api repos/o/r/git/refs/heads/main", "worktree-x"),
