#!/bin/sh
# UserPromptSubmit (общий для всех проектов скелета, ставится install-global.sh;
# в проекте лежит обёртка .claude/hooks/preflight.sh, которая вызывает этот файл).
# Блокирует работу, если защита не установлена.
...
# Проверяем репозиторий проекта, а не $PWD: агент мог сделать cd за пределы
# репо или удалить worktree, в котором стоял.
# Путь бывает относительным (bootstrap.sh) или абсолютным (…/.githooks).
case "$(git -C "${CLAUDE_PROJECT_DIR:-.}" config core.hooksPath 2>/dev/null)" in
  .githooks|*/.githooks) : ;;
  *)
